...
def create_fake_facility_item_type(client):
    ...
    # Cleanup
    try:
        FacilityItemTypeAPI(client).delete_item_type(id=facility_item_type_id, should_return_json=False)
        InventoryItemTypeAPI(client).delete_item_type(id=inventory_item_type_id, should_return_json=False)
